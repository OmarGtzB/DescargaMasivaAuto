...
namespace sw.descargamasiva
{
    internal class Solicitud : SoapRequestBase
    {
        ...
        public string Generate(X509Certificate2 certificate, string rfcEmisor, string rfcReceptor, string rfcSolicitante, string fechaInicial = "", string fechaFinal = "", string tipoSolicitud = "")
        {
        ...
        public override string GetResult(XmlDocument xmlDoc)
        {
            ...
            int estado = Convert.ToInt16(xmlDoc.GetElementsByTagName("SolicitaDescargaResult")[0].Attributes["CodEstatus"].Value);
            if (estado == 5000)
            {
                s = xmlDoc.GetElementsByTagName("SolicitaDescargaResult")[0].Attributes["IdSolicitud"].Value;
            }
            mensajeSolicitud = Convert.ToString(xmlDoc.GetElementsByTagName("SolicitaDescargaResult")[0].Attributes["Mensaje"].Value);
            Console.WriteLine("Estado Solicitud: " + mensajeSolicitud);
            return s;
        }
    }
}
